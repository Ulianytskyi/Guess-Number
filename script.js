...
const guessedNumberRef = document.getElementById("guessed-nums");
...
const play = () => {
    ...
    if (userGuess != answer) {
        ...
        guessedNumberRef.innerHTML = `<span>Guessed Numbers are: </span> ${guessedNumsArr.join(",")}`;
        hint.classList.remove("error");
        setTimeout(()=>{
            hint.classList.add("error");
        }, 10);
    } else {
        hint.innerHTML = `Congratulation!<br>The number was <span> ${answer}</span>.<br>You guesses the number in <span>${noOfGuesses}</span> triess.`;
